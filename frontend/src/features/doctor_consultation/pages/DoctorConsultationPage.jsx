...
export const DoctorConsultationPage = () => {
  ...
  const [shouldScheduleAppointment, setShouldScheduleAppointment] = useState(false);
  const [scheduledInterval, setScheduledInterval] = useState('3_meses');
        ...
        try {
          ...
          if (answers) {
            ...
            patientInfo = {
              ...
              email: appointment.patient_email || loadedHistory.email || "",
      ...
      <div className="bg-white dark:bg-gray-800 p-4 py-8 md:p-8 rounded-lg border border-gray-200 dark:border-gray-700 text-center">
        ...
        <div className="flex flex-col gap-4">
          <button
            ...
            className="text-white py-3 px-8 rounded-lg hover:opacity-90 transition-colors font-medium w-full"
          ...
          <button
            ...
            className="bg-white dark:bg-gray-700 text-black dark:text-white border-2 border-black dark:border-gray-500 py-3 px-8 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors font-medium w-full"
          >
  ...
  const handleSaveConsultation = async () => {
    ...
    const payload = {
      ...
      full_name: String(patientData.full_name || "Desconocido"),
      ci: String(patientData.ci || "N/A"),
      age: patientData.age ? String(patientData.age) : "N/A",
      phone: String(patientData.phone || "N/A"),
      address: String(patientData.address || "N/A"),
      occupation: String(patientData.occupation || "N/A"),
      email: String(patientData.email || ""),

      // Pre-consultation
      reason_for_visit: String(patientData.reason_for_visit || "Control"),
      family_history_mother: String(Array.isArray(patientData.family_history_mother) ? patientData.family_history_mother.join(', ') : (patientData.family_history_mother || "Niega")),
      family_history_father: String(Array.isArray(patientData.family_history_father) ? patientData.family_history_father.join(', ') : (patientData.family_history_father || "Niega")),
      personal_history: String(Array.isArray(patientData.personal_history) ? patientData.personal_history.join(', ') : (patientData.personal_history || "Niega")),
      supplements: String(Array.isArray(patientData.supplements) ? patientData.supplements.join(', ') : (patientData.supplements || "Niega")),
      surgical_history: String(Array.isArray(patientData.surgical_history) ? patientData.surgical_history.join(', ') : (patientData.surgical_history || "Niega")),
      summary_gyn_obstetric: String(formatFullGynObstetricSummary(patientData) || "Sin hallazgos"),
      summary_functional_exam: String(patientData.summary_functional_exam || "Sin hallazgos"),
      summary_habits: String(patientData.summary_habits || "Sin hallazgos"),
      habits_smoking: String(patientData.habits_smoking || patientData.preconsulta_answers?.habits_smoking || patientData.preconsulta_answers?.['15'] || "Niega"),
      habits_alcohol: String(patientData.habits_alcohol || patientData.preconsulta_answers?.habits_alcohol || patientData.preconsulta_answers?.['16'] || "Niega"),
      habits_physical_activity: String(patientData.habits_physical_activity || patientData.preconsulta_answers?.habits_physical_activity || patientData.preconsulta_answers?.['17'] || "Niega"),
      habits_substance_use: String(patientData.habits_substance_use || patientData.preconsulta_answers?.habits_substance_use || patientData.preconsulta_answers?.['18'] || "Niega"),
    ...
    try {
      ...
      // Handle Scheduled Appointment if requested
      if (shouldScheduleAppointment && scheduledDate) {
        try {
          await scheduledAppointmentService.create({
            patient_ci: String(patientData.ci || "N/A"),
            patient_name: String(patientData.full_name || "Desconocido"),
            patient_email: patientData.email || null,
            patient_phone: patientData.phone || null,
            scheduled_date: new Date(scheduledDate).toISOString(),
            interval_type: scheduledInterval,
            notes: scheduledNotes,
            original_consultation_id: newConsultationId,
            original_appointment_id: appointmentId ? parseInt(appointmentId) : null
          });
          console.log("Next appointment scheduled successfully");
        } catch (schedError) {
          console.error("Error scheduling next appointment:", schedError);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-4xl mx-auto p-4 px-0 md:p-6">
        ...
        <div className="space-y-8">
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>1</span>
              Examen Físico
            </h2>
            {renderExamContent()}
          </section>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>2</span>
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>3</span>
              Diagnóstico
            </h2>
            <ListBuilder
              value={diagnosis}
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>4</span>
              Plan
            </h2>
            <ListBuilder
              value={plan}
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>5</span>
              Observaciones
            </h2>
            <ListBuilder
              value={observations}
          ...
          {/* 5.1 Programar Próxima Cita */}
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border-2 border-dashed border-indigo-100 dark:border-indigo-900/30 overflow-hidden relative">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <FiCalendar className="w-24 h-24 text-indigo-600" />
            </div>
            
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold flex items-center gap-2 dark:text-white">
                <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0 bg-indigo-600">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  type="checkbox" 
                  checked={shouldScheduleAppointment}
                  onChange={(e) => setShouldScheduleAppointment(e.target.checked)}
            ...
            {shouldScheduleAppointment && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Intervalo sugerido</label>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        { id: '1_mes', label: '1 Mes' },
                        { id: '3_meses', label: '3 Meses' },
                        { id: '6_meses', label: '6 Meses' },
                        { id: '1_año', label: '1 Año' }
                      ].map((opt) => (
                        <button
                          key={opt.id}
                          ...
                          className={`py-2 px-3 text-sm rounded-lg border transition-all ${
                            ...
                            : 'bg-white border-gray-200 text-gray-600 hover:border-indigo-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400'
                          }`}
                        >
                          {opt.label}
                        </button>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full p-2.5 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notas de seguimiento</label>
                  <textarea 
                    value={scheduledNotes}
                    onChange={(e) => setScheduledNotes(e.target.value)}
                    placeholder="Ej: Revisar resultados de laboratorio, control de eco..."
                    className="w-full p-2.5 h-[120px] bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
              </div>
            )}
          ...
          <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
              <span className="text-white w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0" style={{ backgroundColor: primaryColor }}>
