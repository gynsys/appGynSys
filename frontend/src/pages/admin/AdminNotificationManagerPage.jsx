                                ...
                                <TableRow key={log.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            ...
                                            {log.notification_type === 'doctor_new_contact_message' && log.event_metadata?.patient_phone && (
                                                <button
                                                    onClick={() => {
                                                        const phone = log.event_metadata.patient_phone.replace(/\D/g, ''); // Limpiar el número
                                                        const name = log.event_metadata.patient_name || "";
                                                        const slug = log.event_metadata.doctor_slug || "";
                                                        const url = `https://wa.me/${phone}?text=Hola%20${encodeURIComponent(name)}!%20Recibí%20tu%20solicitud.%20Agenda%20aquí:%20https://gynsys.net/${slug}/onboarding`;
                                                        window.open(url, "_blank");
                                                    }}
                                                    title="Responder por WhatsApp"
                                                    className="ml-2 flex items-center gap-1 px-2 py-1 bg-green-50 text-green-600 hover:bg-green-100 border border-green-200 rounded text-[10px] font-bold transition-colors"
                                                >
