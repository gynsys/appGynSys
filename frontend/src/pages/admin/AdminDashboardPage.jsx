  ...
  return (
    <div className="px-4 py-8 sm:px-6 lg:px-8">
      ...
      {tenants.filter(t => t.status === 'pending').length > 0 && (
        <div className="mb-8 bg-indigo-50 border-2 border-indigo-200 rounded-3xl p-6 flex flex-col md:flex-row items-center justify-between shadow-xl shadow-indigo-100 animate-pulse-subtle">
          <div className="flex items-center mb-4 md:mb-0">
            <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center mr-4 shadow-lg shadow-indigo-200">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <div>
              <h2 className="text-xl font-black text-gray-900">Solicitudes Pendientes</h2>
              <p className="text-indigo-600 font-bold">Tienes {tenants.filter(t => t.status === 'pending').length} médico(s) esperando aprobación.</p>
            </div>
          </div>
          <a
            href="/admin/tenants?status=pending"
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-8 py-3 rounded-2xl shadow-lg shadow-indigo-200 transition-all transform hover:scale-105"
          >
            Revisar Solicitudes
          </a>
        </div>
